...
fn collect_src_files(dir: &PathBuf) -> Vec<PathBuf> {
    walkdir::WalkDir::new(dir)
        ...
        .filter_map(|e| e.ok())
        .map(|e| e.into_path())
        .filter(|path| path.is_file() && path.extension().map_or(false, |ext| ext == "c" || ext == "h"))
        .collect()
}
...
async fn main() -> anyhow::Result<()> {
    ...
    for file in files {
        ...
        let handle = tokio::spawn(async move {
            read_and_process_file(file, shared_struct_map, shared_dwarf, flags_regex_str).await.unwrap();
            drop(permit);
        });
        handles.push(handle);
    }
...
fn apply_highlight_ranges(content: &mut String, base_range: &std::ops::Range<usize>,
...
fn display_match(content: &Vec<u8>, path: &PathBuf, struct_: &dwat::Struct,
                 dwarf: &Arc<RwLock<dwat::dwarf::OwnedDwarf>>, qm: &QueryMatch) {
    ...
    let decl_line_start = byte_offset_to_line_number(&content, qm.function_definition.byte_range().start).unwrap();
    ...
    let mut function_src = qm.function_definition.utf8_text(content).unwrap().to_string();

    // find line index opening brace is on
    let brace_location = function_src.find('{')
        .expect("no opening brace in function source?");
    let end_decl_line = byte_offset_to_line_number(&content[decl_line_start..].to_vec(), brace_location)
        .expect("failed when looking for line number of opening brace") - 1;
    ...
    println!("");
    if std::io::stdout().is_terminal() {
        println!("\x1b[1m{}\x1b[0m:{}", path.to_str().unwrap(), decl_line_start);
    } else {
...
async fn process_file_content(path: PathBuf, content: Vec<u8>, struct_map: Arc<RwLock<HashMap<String, dwat::Struct>>>,
                              dwarf: Arc<RwLock<dwat::dwarf::OwnedDwarf>>, flags_regex_str: Option<String>)
-> Result<(), Box<dyn std::error::Error>>
{
    let mut parser = TsParser::new();

    parser.set_language(ts_c::language())
          .expect("Error loading C grammar");

    let parsed = parser.parse(&content, None).expect("Parser returned no tree");
    ...
    for match_ in matches {
        ...
        let struct_name = captures.get(2).unwrap().node.utf8_text(&content).unwrap_or("").to_string();
        ...
        if let Some(struct_) = struct_map.get(&struct_name) {
            ...
            let flags = captures.get(7).unwrap().node.utf8_text(&content).unwrap_or("").to_string();
...
async fn read_and_process_file(path: PathBuf, struct_map: Arc<RwLock<HashMap<String, dwat::Struct>>>,
                               dwarf: Arc<RwLock<dwat::dwarf::OwnedDwarf>>, flags_regex_str: Option<String>)
-> Result<(), Box<dyn std::error::Error>>
{
    ...
    let _ = task::spawn_blocking(move || process_file_content(path, contents, struct_map, dwarf, flags_regex_str)).await?.await;
    Ok(())
}
